...
## Prompt as Code — Why This Changes Everything

Traditional prompt engineering means editing giant text files that no one can review. AgentFlowBuilder treats agent workflows as **structured data** — version-controlled, visually editable, and automatically quality-checked.

### Version-Controlled Workflows

Workflows are stored as clean JSON files in `./workflows/`. This means your agent logic gets the same engineering rigor as your application code:

| | Hand-Written Prompts | AgentFlowBuilder JSON |
|---|---|---|
| **Git diff** | Wall of text — impossible to spot what changed | Structured diff — see exactly which node, edge, or condition changed |
| **Code review** | Reviewers skim or skip long prompt files | Each node is a discrete unit — reviewers can focus on specific logic |
| **Branching** | Copy-paste prompt variants, lose track fast | Branch the workflow file, merge when the experiment wins |
| **Merge conflicts** | Manual line-by-line resolution | JSON structure makes conflicts localized and resolvable |
| **Rollback** | Hope you saved the old version somewhere | `git revert` — done |

### Visual Flow = Living Documentation

The canvas isn't just an editor — it's documentation that stays in sync with the actual workflow:

- **See the entire flow at a glance** — no scrolling through pages of prompt text to understand the logic
- **Insert steps by dragging** — need an extra validation step? Drag a node between two existing ones
- **Branching logic is visible** — Condition nodes show True/False paths as actual visual branches, not buried `if` clauses in text
- **New team members onboard instantly** — point them at the canvas instead of asking them to read a 2,000-word prompt file
- **Mermaid diagrams auto-generated** — export a visual diagram for documentation, wikis, or PRs with one click

### Real-Time Sync Between Canvas and AI

The web UI and MCP tools share the same `./workflows/` directory with **live SSE (Server-Sent Events) synchronization**. Edit a workflow on the canvas → your AI assistant sees the change instantly via MCP. Your AI modifies a workflow via MCP tools → the canvas updates in real-time. No manual refresh, no stale state — one source of truth.

### Cross-Project Skill Discovery

When you publish a skill, it goes to `~/.claude/skills/{name}/` — a standard location that Claude Code discovers automatically in **any project**. Build a code review skill in your main repo, and it's instantly available in every other project on your machine. Each published skill includes:

- `SKILL.md` — the production-ready skill file
- `references/workflow.json` — the original workflow for traceability
- `references/grading.json` — quality scores and grading history

### Smart Auto-Processing

AI-generated workflows aren't always clean. The post-processing pipeline automatically:

- **Slugifies node IDs** — converts names to safe identifiers (`Review PR` → `review_pr`)
- **Detects entry points** — finds nodes that aren't referenced by others and orders topologically
- **Auto-layouts the canvas** — places nodes in a readable grid without manual arrangement
- **Rebuilds edges** — reconstructs visual connections from the node logic, ensuring canvas fidelity
- **Labels condition branches** — automatically adds True/False labels in your language

### Hierarchical Disclosure — No More Context Decay

Long prompts suffer from context decay: the AI forgets instructions from the top when it reaches the bottom. Every skill generated by AgentFlowBuilder uses **Hierarchical Disclosure** — breaking work into stages where the AI focuses on one micro-task at a time, loading only the context it needs for that step. The result: reliable execution even for complex, multi-step workflows.

### Why Generated Skills Beat Hand-Written Prompts

| | Hand-Written Prompts | AgentFlowBuilder Skills |
|---|---|---|
| **Version control** | Text blobs that are hard to diff | Structured JSON — meaningful diffs, easy merges |
| **Visual review** | Read the entire prompt to understand flow | Glance at the canvas to see the full picture |
| **Quality assurance** | No standard — quality varies by author | 6-pillar quality gate auto-grades and iterates until score ≥ 80/100 |
| **Reusability** | Copy-paste between projects, diverges over time | Publish once to `~/.claude/skills/`, discovered everywhere |
| **Portability** | Locked to one IDE or tool | Export to Claude Code, Cursor, or Antigravity in one click |
| **Team collaboration** | "Here's my prompt, good luck" | Shared repo of workflow JSONs — PR, review, merge |
| **Traceability** | No history of why a prompt looks the way it does | Workflow JSON + grading history stored with every published skill |
| **Context decay** | Entire prompt loaded at once, tail instructions forgotten | Hierarchical Disclosure — staged execution, focused context |
