...
- **`.mjs` interop flag (`AGENTFLOW_MJS`)**: off by default. Set `AGENTFLOW_MJS=1` to enable Claude Code dynamic-workflow `.mjs` support — e.g. `AGENTFLOW_MJS=1 npm start`. When **on**, `.mjs` joins `listFormats()` (so it participates in bare-name load precedence / list / delete), `/api/import` parses `.mjs` natively, `/api/capabilities` reports `mjs:true`, and the UI shows `.mjs` in the export/save/import pickers. When **off**, a stray `name.mjs` is invisible to bare-name load (no silent shadowing) and the UI hides `.mjs`. **Explicit ops are never gated**: `serializeByFormat(w,'mjs')`, MCP `export_workflow` with `mjs`, and `load("name.mjs")` (extension given) always work. The flag is read once at startup (`process.env`), so changing it requires a server restart. `.mjs` export is approximate-only; import is best-effort (collapses loops/branches/fan-out to `ScriptExecution` nodes + warnings).
